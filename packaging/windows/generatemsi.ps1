...
function RunCandle
{
    ...
    .\candle.exe -dDotnetSrc="$inputDir" `
        ...
        -dDisplayVersion="$env:DOTNET_BUILD_VERSION" `
        -arch x64 `
        -ext WixDependencyExtension.dll `
        "$AuthWsxRoot\dotnet.wxs" `
        "$AuthWsxRoot\provider.wxs" `
        "$AuthWsxRoot\registrykeys.wxs" `
        "$AuthWsxRoot\arpdisplayversion.wxs" `
...
$DotnetMSIOutput = Join-Path $PackageDir "dotnet-win-x64.$env:DOTNET_MSI_VERSION.msi"
